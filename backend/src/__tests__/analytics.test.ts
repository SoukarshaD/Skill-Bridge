...
describe("Analytics API", () => {
  ...
  it("should get placement funnel calculating historical reach", async () => {
    ...
    console.log("History 3, calling supertest...");

    const res = await request(app)
      .get("/api/analytics/institution/funnel")
      .set("Cookie", [`token=${adminToken}`]);

    console.log("Supertest returned", res.status);
    expect(res.status).toBe(200);
    expect(res.body.APPLIED).toBe(1);
    expect(res.body.SHORTLISTED).toBe(1);
    expect(res.body.INTERVIEW).toBe(1);
    expect(res.body.OFFERED).toBe(1);
    expect(res.body.ACCEPTED).toBe(0);
  }, 90000);

  it("should get separate offered and accepted outcomes by department", async () => {
    await prisma.application.create({
      data: {
        opportunityId: oppId,
        studentId: student1Id, // CSE
        status: "OFFERED"
      }
    });
    ...
    await prisma.application.create({
      data: {
        opportunityId: oppId,
        studentId: student2Id, // ECE
        status: "ACCEPTED"
      }
    });

    const res = await request(app)
      .get("/api/analytics/institution/outcomes")
      .set("Cookie", [`token=${adminToken}`]);

    expect(res.status).toBe(200);
    const cse = res.body.find((o: any) => o.department === "CSE");
    const ece = res.body.find((o: any) => o.department === "ECE");

    expect(cse.offered).toBe(1);
    expect(cse.accepted).toBe(0);
    expect(ece.offered).toBe(0);
    expect(ece.accepted).toBe(1);
  }, 90000);
