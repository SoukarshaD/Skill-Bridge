...
import { Plus, Clock, CheckCircle2, XCircle, ArrowRight, BookOpen } from "lucide-react";
...
export default function AcademicianCollaborations() {
  ...
  useEffect(() => {
    const fetchCollaborations = async () => {
      try {
        const data = await api.get<any>("/collaborations/academician");
          ...
          <div className="flex flex-col md:flex-row md:items-end justify-between gap-4 mb-8">
            ...
            <Link href="/academician/collaborations/new">
              <Button className="h-9 shadow-sm">
                <Plus className="mr-2 h-4 w-4" /> Propose Collaboration
              </Button>
            </Link>
          </div>
              ...
              {isLoading ? (
                ...
                <div className="space-y-4">
                  {collaborations.map((collab) => {
                    ...
                    return (
                      <div key={collab.id} className="flex flex-col sm:flex-row sm:items-center justify-between gap-4 p-5 rounded-xl border border-border/50 bg-background hover:border-indigo-500/30 transition-colors">
                        <div className="flex items-start gap-4">
                          <div className="w-12 h-12 rounded-lg bg-indigo-500/10 flex items-center justify-center shrink-0">
                            <BookOpen className="h-6 w-6 text-indigo-600" />
                          </div>
                          <div>
                            <Link href={`/academician/collaborations/${collab.id}`}>
                              <h4 className="font-semibold text-lg hover:underline cursor-pointer">
                                {collab.title}
                              </h4>
                            </Link>
                            <p className="text-sm font-medium text-foreground mt-1">
                              To: {collab.industry?.name || 'Unknown Company'}
                            </p>
