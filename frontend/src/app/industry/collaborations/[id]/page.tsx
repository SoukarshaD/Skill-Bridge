...
import { Clock, CheckCircle2, XCircle, UserCircle, Briefcase, FileText, ChevronLeft, Loader2 } from "lucide-react";
import { cn } from "@/lib/utils";
import { toast } from "sonner";

export default function IndustryCollaborationDetail() {
  const { id } = useParams();
      ...
      <div className="min-h-screen bg-muted/20 flex flex-col font-sans">
        ...
        <main className="flex-1 container mx-auto px-4 md:px-8 py-8 max-w-5xl">
          ...
          <div className="grid md:grid-cols-3 gap-6">
            ...
            {/* Right Column: Academician Info & Actions */}
            <div className="space-y-6">
              <Card className="shadow-sm border-border/50">
                ...
                <CardContent className="pt-6">
                  <h3 className="font-semibold text-lg">{collab.academician?.name}</h3>
                  <p className="text-sm text-muted-foreground mt-1">
                    {collab.academician?.academicProfile?.department || 'Faculty'}
                  </p>
                  <p className="text-sm font-medium mt-1">
                    {collab.academician?.academicProfile?.institution || 'Unknown Institution'}
                  </p>
                  ...
                  <div className="mt-6 space-y-4">
                      ...
                      <div className="flex flex-wrap gap-1.5 mt-2">
                        {collab.expertise.length > 0 ? (
                          collab.expertise.map((exp: string, i: number) => (
                            <Badge key={i} variant="outline" className="text-xs bg-muted/30">{exp}</Badge>
                          ))
                        ) : (
                    ...
                    <div>
                      ...
                      <p className="text-sm font-medium mt-1">{collab.duration || 'Not specified'}</p>
                    </div>
                  </div>
                </CardContent>
