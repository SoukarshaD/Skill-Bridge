...
export default function StudentProgramsPage() {
  ...
  const fetchData = async () => {
    try {
      const [programs, regs, recs] = await Promise.all([
        api.get<any[]>("/programs"),
        api.get<any[]>("/programs/my-registrations"),
        api.get<any[]>("/programs/recommendations")
      ]);
      setAllPrograms(programs || []);
      setMyRegistrations(regs || []);
      setRecommendedPrograms(recs || []);
    } catch (e) {
      console.error(e);
    }
  };
  ...
  const renderProgramCard = (p: any, matchInfo?: any) => {
    return (
      <Card key={p.id} className="flex flex-col h-full hover:shadow-md transition-shadow">
        ...
        <CardFooter className="pt-4 border-t mt-auto">
          <Link href={`/student/programs/${p.id}`} className="w-full">
      ...
      <Tabs defaultValue="browse" className="space-y-8">
        ...
        <TabsContent value="recommended" className="space-y-6">
          {recommendedPrograms.length > 0 ? (
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-6">
              {recommendedPrograms.map(r => renderProgramCard(r.program, r.match))}
            </div>
          ) : (
            <div className="text-center py-12 border rounded-lg bg-muted/20">
              <p className="text-muted-foreground">No recommendations available yet. Complete your profile and add skills!</p>
            </div>
          )}
        </TabsContent>
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-6">
              {myRegistrations.map(reg => (
                <Card key={reg.id} className="flex flex-col h-full hover:shadow-md transition-shadow">
                  ...
                  <CardFooter className="pt-4 border-t mt-auto">
                    <Link href={`/student/programs/${reg.program.id}`} className="w-full">
                      <Button variant="outline" className="w-full">View Details</Button>
                    </Link>
                  </CardFooter>
                </Card>
